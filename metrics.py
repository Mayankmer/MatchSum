...
class ValidMetric(MetricBase):
    ...
    def evaluate(self, score):
      ...
      # Fast ROUGE using raw text and summary
      for i in range(batch_size):
          max_idx = int(torch.max(score[i], dim=0).indices)
          if max_idx >= len(self.data[self.cur_idx]['candidates']):
              self.Error += 1
              self.cur_idx += 1
              continue
          # Get decoded summary and reference
          dec = self.data[self.cur_idx]['candidates'][max_idx]
          ref = self.data[self.cur_idx]['summary']
...
class MatchRougeMetric(MetricBase):
    ...
    def evaluate(self, score):
      max_idx = int(torch.max(score, dim=1).indices)
      self.ext.append(max_idx)
      self.cur_idx += 1
      print('{}/{} ({:.2f}%) decoded in {} seconds\r'.format(
            self.cur_idx, self.n_total, self.cur_idx/self.n_total*100, 
            timedelta(seconds=int(time()-self.start))), end='')

    def get_metric(self, reset=True):
        print('\nStart writing files !!!')
        for i, ext in enumerate(self.ext):
            # Use raw text and summary from the dataset
            dec = self.data[i]['candidates'][ext]
            ref = self.data[i]['summary']
            
            with open(join(self.dec_path, f'{i}.dec'), 'w') as f:
                print(dec, file=f)
            with open(join(self.ref_path, f'{i}.ref'), 'w') as f:
                print(ref, file=f)
